    ...
    <div class="row"><div class="col-sm-12"><table id="zero_config" class="table table-striped table-bordered dataTable" role="grid" aria-describedby="zero_config_info">
        ...
        @foreach ($supplier as $item)
             <tr role="row" class="odd">
                <td class="sorting_1">{{$item->name_supplier}}</td>
                <td>
                    <a href="supplier/edit/{{$item->id}}"><i class=" fas fa-pencil-alt"></i> Edit</a>
                    <a href="supplier/delete/{{$item->id}}"><i class=" fas fa-trash"></i> Delete</a>
                </td>
            </tr>
        @endforeach
        </tbody>
